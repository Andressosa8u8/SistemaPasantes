...
namespace CapaNegocio
{
    ...
    public class cnPasantes
    {
        ...
        public static Tbl_Pasantes ObtenerAcuerdoPasante(int idPasante)
        {
            var pasante = dc.Tbl_Pasantes.FirstOrDefault(pasa => pasa.Pasantes_id.Equals(idPasante));
            return pasante;
        }
        ...
        public static (int HorasNecesarias, int HorasCumplidas) ObtenerHorasPasante(int idPasante)
        {
            using (var context = new DataClasses1DataContext())
            {
                var result = context.ObtenerHorasNecesariasxHorasCumplidas(idPasante).FirstOrDefault();
                if (result != null)
                {
                    return (HorasNecesarias: result.HorasNecesarias ?? 0,
                            HorasCumplidas: Convert.ToInt32(result.Horas_Cumplidas ?? 0));
                }
                else
                {
                    return (HorasNecesarias: 0, HorasCumplidas: 0);
                }
            }
        }
    }
